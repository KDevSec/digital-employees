...
**Architecture:** 绿地新增 `src/installs/`（spec/registry/negotiate/manifest/executor/report/launch/uninstall）+ `src/adapters/`（contract + 三底座）+ `src/bases/`，路由挂 I0-5 分域注册表。安装 = `negotiate() → adapter.plan() → executor.execute()`（事务八条）；身份锚三底座统一 config-domain（D-L2-01），project-file 为回退档。
