...
**Architecture:** 绿地新增 `src/installs/`（spec/registry/negotiate/manifest/executor/report/launch/uninstall）+ `src/adapters/`（contract + 三底座）+ `src/bases/`，路由挂 I0-5 分域注册表。安装 = `negotiate() → adapter.plan() → executor.execute()`（事务八条）；身份锚三底座统一 config-domain（D-049），project-file 为回退档。
...
describe('三底座档案（设计 §5.1；TS 常量——bun --compile 单体约束）', () => {
  ...
  it('身份锚三底座统一 config-domain（D-049 主路径）', () => {
    for (const p of Object.values(baseProfiles)) {
...
```ts
/** Qoder 档案（config-domain 主路径 D-049；project-file 回退档改本档案值即可，零代码） */
import type { BaseProfile } from '../contract'
...
describe('三底座 plan()（设计 §5 落位；config-domain 主路径）', () => {
  ...
  it('Qoder：同构 CC，身份文件 AGENTS.md（config 域内，D-049 主路径）', async () => {
 ...
 * project-file 回退档（D-049 档案值切换）：身份 target = workdir 根 AGENTS.md 薄壳位、skills 带 ds-<id>- 前缀。
 */
import { join } from 'node:path'
