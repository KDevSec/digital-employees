...
**状态：代码全量合入 main 并推送 origin（365af0f）；单元测试全绿；VM 全链验证按用户裁决交接他人。**

- 合并轨迹：#1 `9dda303`（T1-14 模块层，含与 L3 引擎线契约语义合并：Ctx.query 统一 URLSearchParams / toHonoApp opts{mcpServer,sessionGuard} 并集 / Res.stream 与 redirect+cookies 并存）→ #2 `7bd8d17`（T15-16 接线：路由九域并集 + main 装配 + routes-engine/assembly 装配补齐）→ origin 同步两笔（demo 删除 681b471 + 平台部署线 fb9a127）→ 推送 `365af0f`
- 测试：service **319/319**（44 文件）+ engine 115/115 + web 350/350（web 有 L5 线带入的既有 unhandled rejection 噪声——EventWatchPanel scrollTo，非本线）+ typecheck 0 错；满载偶发超时加固两处（L3 driver 用例余量放宽 / 本线 heartbeat settleUntil）
- 冒烟锚①（开发态全放行 + 生产语义切换 + guard 401 + clean stop）：T15-16 实现者真机自跑全过（task-15-16-report.md）

### VM 全链验证交接清单（接手人执行）

1. **环境准备**：VM Keycloak `workbench-desktop` client redirectUris 加 `http://localhost:19990/auth/callback` + `http://127.0.0.1:19990/auth/callback`（或本机 vite 5174 代理 + `VITE_PROXY_TARGET=19990`，白名单已有）；确认平台 nginx `.well-known/` 与 `/oauth2/` 反代补丁仍在（容器重建即丢需重打）；注意 origin/main `fb9a127` 起 Keycloak 全链 HTTPS 部署有变——地址形态以最新部署为准
2. **验收锚②生产语义全链**：`WORKBENCH_HOME=<临时 profile> bun run workbench/workbench-service/src/main.ts start --foreground`（19990）→ PUT `/api/config/platform` 配平台地址 → 未登录业务 API 401 → 浏览器登录（Keycloak `test`/`test1234`）→ 自动提交申请（PENDING_REVIEW）→ `system.admin`/`Horse~test@2026` 平台审批 → 前端 5s 轮询 `/api/progress` 推进 → challenge/proof/complete 激活 → **后台心跳自动 ACTIVE（60s 定时器，真时间戳）** → 顶栏「平台已连接」
3. **验收锚③重启会话保持**：登录态下 kill 服务 → 重启（同 profile）→ 浏览器免重登（`auth/sessions.enc` 持久化）；installationId 不变
4. **参考**：设计文档 §9.3 + i0-webshell.md VM 部署补记（65b7b69）；demo 已从 main 删除，对照实现以分支历史 `feat/auth-migration` 为准

### 遗留事项（待用户/后续会话处置）

- **契约回写清单**（设计文档 §8：登录 v0.2 §5.2 GET→POST 笔误回写 + /api/reset 补行 + D-15/D-049 注记修订 + 占位号 D-am1~5 转真号 ≥D-062——main 决策号已用至 D-061：D-056~058=L3、D-059=L2、D-060/061=L5×L3 联调线（2026-08-27 17:00 step-recorder 实证撞号后顺延））
- Minor 延期清单（终审未跑，按用户裁决免除）：原子写惯用法统一 / store 并发窗口 / ensureRunning 同步幂等加固（3 行修法在 ledger）/ redirect+cookies 组合无专测 / redirect 未约束 3xx
- `feat/auth-migration` 分支与 `.worktrees/auth-migration` 工作区：已全量合入，可清理（决策归用户）
- web 基线 unhandled rejection（L5 线 EventWatchPanel）提请看板线修复
