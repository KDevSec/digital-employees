...
| 决策号 | 合流已转真号 D-050~D-055（D-049 被 main 平台地址裁决占用顺延；见 §12 与决策记录） |
...
## 12. 决策记录（合流转真号：D-050~D-055 已落（D-049 被 main 侧平台地址开发环境裁决占用，顺延）[决策记录](../design/设计决策记录-2026-08-13.md)）

| 真号 | 决策 |
|---|---|
| D-050 | SSE 消费层 EventSourceLike 可注入 + 连接状态机 + seq 按 task_id 分域去重（原占位 D-kb01，含走查实捕修复） |
| D-051 | 归并层 applyEvent 纯函数 + 剧本重放测试 = 看板行为规格（原占位 D-kb02） |
| D-052 | 表快照 + 员工映射经 getTask 随任务下发（原占位 D-kb03，契约歧义 A/B 先行口径） |
