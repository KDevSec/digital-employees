...
## 7 T8 增量设计：平台地址配置（2026-08-26 用户需求追加）

> 需求原文：工作台首页配一个 IP 端口（管控平台地址），打包时预填默认值，用户可改、改后存本地配置——目的：前期平台服务器地址频繁切换。用户裁决采**方案 A**（配置基础设施本线做，消费链路归 A 系列，G-1 不变）。

| # | 决策 | 说明 |
|---|------|------|
| D-13 | 存储 = config.json `platform.baseUrl`（S-07 基础设施层） | 与端口/更新源同类（影响平台发现与更新源），不进 settings.json（用户偏好层）。代码默认 `http://127.0.0.1:18000`（与 demo 一致）；写入只落覆盖键、保留既有键 |
| D-14 | 端点 = config 域 `routes/config.ts`（GET/PUT `/api/config/platform`） | 域划分对齐详设 §10.2（settings 归 config 模块，未来 /api/settings 同域）；PUT zod 校验合法 http(s) URL；GET 每次重读 config 文件（PUT 后立即可见）；PUT 合并写不覆盖其他键 |
| D-15 | 鉴权暂无（与 healthz 同档），A 系列补档 | 会话机制不存在（G-1）；本机边界 = S-12 仅绑 127.0.0.1 + Host 白名单。端点注释标注「A 系列鉴权中间件落地后升『会话』档」（详设 §10.2 settings 行同档规划） |
| D-16 | UI = 接入页「平台连接」配置卡（F-03 增量） | 显示当前平台地址 + 行内编辑（input + 保存）+ 校验失败提示 + 保存成功反馈；位置放 hero 区（登录引导语旁），未登录即可配（登录前就要知道平台在哪） |
| D-17 | 打包默认值 = 安装器写覆盖，机制本线留好 | 代码默认值即「不配置时的兜底」；安装期预填由安装器线写 config.json 覆盖键（跨线协作点，本线不碰 installer） |
| D-18 | 消费边界：本线不消费 | 登录/enrollment 读该地址属 A-01（G-1 已档）；demo 验收链路继续用 env。UI 配置的即时效果 = 保存/持久化，非登录链路切换 |

## 8 T9 增量设计：登录页重设计 + 登录动线闭环（2026-08-26 用户头脑风暴裁决）

> 背景：用户验收发现动线断层（登录后无路可走）+ 接入页「不像登录页」（登录按钮无主角感、安全边界卡是 demo 开发者展示物）。参照 QoderWaker 登录.png/设置.png（视觉参考：品牌感装饰、设置入口承载退出），用户拍板：形态 A + 背景装饰 + 登录自动跳转 + 设置按钮退出。

| # | 决策 | 说明 |
|---|------|------|
| D-19 | 未登录 = 居中单卡登录页 | 白卡（~400px）：logo 方块（同侧栏款蓝渐变）+ DevZero 品牌名 + 「数字员工工作台」副标 + 引导语 + **整宽放大「企业账号登录」唯一主按钮** + 卡底一行（「平台设置」展开入口收纳 T8 配置卡 + 服务状态·版本小字）。背景 = --bg 底 + 左上/右下原型蓝渐变 radial 光斑（纯 CSS 无资源依赖）。QoderWaker 登录页的二维码/设备码交互不参考（我们是 OIDC 整页跳转） |
| D-20 | 已登录态接入页简化 | 删安全边界卡（demo 时代开发者展示物，用户裁决不需要）；状态卡升主位、平台配置卡次位；布局骨架沿用 |
| D-21 | 登录动线 | 守卫 `resolveRedirect` 扩展三参（authenticated/status/toPath）：已登录且 ACTIVE 且目标为 '/' → 跳 `/employees`（F-02「我的员工（默认）」语义兑现）；已登录非 ACTIVE 访问 '/' → 停接入页（盯审批）；未登录不变（仅 '/'）。注意守卫与自动跳的时序：回调落 '/' 时守卫已有 state（首次导航已拉），直接按状态分流 |
| D-22 | 顶栏设置按钮下拉菜单（非完整设置中心） | TopBar 用户区加齿轮按钮 → 下拉菜单两项：「接入与平台设置」（跳 '/'）+「退出登录」（调 logoutAction → 刷新 state → 守卫拦回 '/' 登录页）。完整设置中心留 S-07（settings.json 用户偏好）落地时升级——用户确认 2026-08-26 |

## 9 T10 增量设计：设置聚合重构（2026-08-26 用户验收指令）

> 用户验收顶栏后指令：「Test User/邮箱/平台已连接/版本/检查更新——第一行这些内容都放到左下角的设置按钮里」。

| # | 决策 | 说明 |
|---|------|------|
| D-23 | 设置按钮落位侧栏底部（原型 sidebar-foot 结构） | SideNav 增 sidebar-foot 区：齿轮按钮（nav-item 同形态竖排图标+「设置」微字）；点击向上弹出设置浮层 |
