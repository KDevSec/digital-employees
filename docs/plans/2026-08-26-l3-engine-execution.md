...
> 基线 → 终态：engine 0 → **115 绿**（108 + 复审修复批 4 + 🟡2 合并自愈批 3）；service 126 → **151 绿**（150 + spawn 前 .mcp.json 键级自愈 1）；web 224 回归不动；全部分包 vitest 口径（根 bun test 扫非 workspace 目录的 163 固有失败与改动无关——T1 时 stash 实测基线即如此）
...
## 7. 🟡2 复审遗留裁决收口（2026-08-27 用户裁决）+ L5 前端参考输入
