...
| 项 | 值 |
|---|---|
| 版本 | v0.3 · QoderWake 实证修正版 |
| 日期 | 2026-08-23（v0.2：2026-08-10） |
| 状态 | 🟡 草案（替代 v0.1 自研中性协议路线，待评审） |
| 取代 | UPP v0.1 草案（未入库，pivot 前 audit trail 仅存于 git 历史）（自研中性协议路线，存档保留作 pivot 前 audit trail） |
| v0.3 变更 | ① §0.5/§4.1 第一类适配面定为「向底座约定目录投递文件」（影子 skill 模式，W-20）；② §5.1 新增 UPP 插件清单 schema 预留（`upp.plugin.v1`，W-19）；③ §7.3 新增硬边界 B-8：底座依赖已装 + 严格版本下限断言，不随包分发 CLI（W-22）。实证来源：[QoderWake 源码拆解](../../research/QoderWake二进制源码结构拆解-对0.1的可借鉴点-2026-08-20.md) §8.8（与 [Multica 调研](../../research/Multica深度调研-底座调用与协同编排-2026-08-20.md) 交叉验证） |
| 依据 | [开放标准与 UPP 再审视调研报告](../../research/数字员工多底座适配调研报告-开放标准与UPP再审视-2026-08-10.md)（行业向开放标准收敛的实查）+ [白皮书](数字员工套件2.0架构白皮书-v0.8.md) §3.7 + 1.0 真实代码盘点 |
...
## §0.5 v0.2 -> v0.3 的核心变化（2026-08-23，QoderWake/Multica 双源实证）

v0.2 定下「开放标准薄封装」后，QoderWake 源码拆解（§8.8）与 Multica 调研提供了三条交叉验证的修正：

| # | 修正 | 内容 | 依据 |
|---|------|------|------|
| W-20 | **第一类适配面 = 文件系统约定** | 「向底座约定目录投递文件」（影子 skill 模式）优先于「调用底座插件 API」--前者对每个底座都成立，后者只对愿意开放的底座成立。QoderWake 影子 skill（投递到 `.agents/skills/`）与 Multica 的 CLAUDE.md/AGENTS.md 投递不谋而合 | 源码拆解 §8.8.2 |
| W-19 | **插件清单 schema 形式** | 若 UPP 定义插件（触发器源等，0.2+）：`schemaVersion` 带命名空间（`upp.plugin.v1`）、`capabilities` 分类嵌套（宿主忽略不认识的分类）、`configSchema`/`uiSchema` 分离（宿主渲染表单）、租户识别字段内建；**禁止插件带前端 bundle** | 源码拆解 §8.8.3 |
| W-22 | **底座依赖已装** | 不随包分发底座 CLI（多底座下 N × 160MB 不可接受）；「探测到的 CLI 版本不在支持区间」是一等安装期错误，不是运行中怪错 | 源码拆解 §8.8.1 |
...
**第一类适配面 = 文件系统约定（v0.3，W-20）**：UPP 的主适配路径是「向底座约定目录投递文件」（影子 skill 模式：身份/能力文件落位到 `.agents/skills/`、`.claude/skills/`、root AGENTS.md 等约定位置），而非依赖底座提供插件 API--前者对每个底座都成立，后者只对愿意开放的底座成立。plugin 后端（marketplace add + plugin install，D-014）是后续可选优化，不是第一类适配面。
...
### 5.1 UPP 插件清单 schema（`upp.plugin.v1`，预留）

员工包 manifest（上文）之外，若 UPP 未来定义**插件**（触发器事件源、agent-loop 类扩展，0.2+；V0.1 不实现），其清单按以下形式（QoderWake `qoderwake-plugin.json` 实证 + Multica 交叉验证，W-19）：

```yaml
schemaVersion: "upp.plugin.v1"     # 带命名空间，一份 JSON 落地自证身份（不用裸版本号）
name / version / displayName / description
capabilities:                      # 分类嵌套：宿主只读自己认识的分类，不认识的整块忽略（能力前向兼容）
  trigger:
    webhookEventSources[]:
      kind / displayName / order
      identity.sourceScopeIdField  # 租户/仓库识别字段内建，多租户从第一天就在 schema 里
      configSchema: { ... }        # 标准 JSON Schema：插件只声明需要哪些配置、长什么样
      uiSchema:                    # 布局声明：宿主负责渲染表单
        layout: [[...], [...]]
        fields.<name>: { widget, label, placeholder }
      defaults: { ... }
```

**硬规则：插件禁止携带前端 bundle**--UI 由宿主按 uiSchema 声明式渲染（QoderWake agent-loops 自带 1MB `ui/management.js` bundle 是反面教材）。
...
| **B-8（v0.3）** | **底座版本不确定**（依赖已装模式下用户装任意版本） | W-22：不随包分发 CLI（多底座 N × 160MB 不可接受）；安装时严格版本探测 + 支持区间下限断言，版本不符 = 一等安装期错误（明确报错），不是运行中怪错 |
...
| 文档 | 关系 |
|------|------|
| [开放标准与 UPP 再审视调研报告](../../research/数字员工多底座适配调研报告-开放标准与UPP再审视-2026-08-10.md) | 本规范的 pivot 依据（§3.4 CC Hooks 事实标准发现 + 方案 C 推荐） |
| UPP v0.1 草案（未入库，pivot 前 audit trail 仅存于 git 历史） | 本规范取代之；v0.1 的三支点/能力协商/降级目录/硬边界保留，自研中性协议层废弃 |
| [白皮书](数字员工套件2.0架构白皮书-v0.8.md) §3.7 | 本规范是其协议层展开（白皮书 v0.8 已对齐 V0.1 裁决批次 D-017~D-033） |
| [QoderWake 源码拆解](../../research/QoderWake二进制源码结构拆解-对0.1的可借鉴点-2026-08-20.md) §8.8 | v0.3 三条修正（W-19/W-20/W-22）的实证来源 |
| [UPP 可行性调研](../../research/多智能体底座插件系统调研-UPP可行性分析-2026-08-08.md) | 八底座插件体系实查，本规范 §6.3 矩阵的数据底座 |
