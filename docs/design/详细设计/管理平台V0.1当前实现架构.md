...
> **工作台端口注记（2026-08-23）**：上图与本节的 `:19820` 描述的是**当前已交付实现**（compose 映射 + auth demo 默认值）。套件工作台 V0.1 产品默认端口已改为 **19980**（本机 19820 被 QoderWake 实占，D-022），仅绑 127.0.0.1；V0.1 联调时 compose 映射与 Keycloak 回调地址随之调整，平台侧代码不变。
...
面向全公司 1000 人同时在线和工作台监控的容量缺口、生产化改造和压测基线，见 [公司级容量与扩展性评估](../../research/管理平台V0.1公司级容量与扩展性评估-2026-08-16.md)。
