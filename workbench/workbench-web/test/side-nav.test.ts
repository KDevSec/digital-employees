 ...
 * - 可点项四枚：我的员工（/employees，默认选中）/ 底座与环境（/bases）/ 协同编排（/collab，
...
const router = createRouter({
  ...
  routes: [
    ...
    { path: '/collab', component: { name: 'EmptyKB', render: () => null } },
...
describe('SideNav 可点导航项（四枚）', () => {
  it('四枚 RouterLink 渲染且 to/文案/锚点正确（/employees /bases /collab /kanban——协同编排与任务看板同级，T4 双层）', () => {
    const wrapper = mountNav()
    const links = wrapper.findAllComponents(RouterLink)
    expect(links.map((link) => link.props('to'))).toEqual(['/employees', '/bases', '/collab', '/kanban'])
    expect(links.map((link) => link.text())).toEqual(['我的员工', '底座与环境', '协同编排', '任务看板'])
    // 锚点形态（可点）：href 与路由路径一致
    const anchors = wrapper.findAll('a')
    expect(anchors).toHaveLength(4)
    expect(anchors.map((anchor) => anchor.attributes('href'))).toEqual(['/employees', '/bases', '/collab', '/kanban'])
  })
