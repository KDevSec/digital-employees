...
/**
 * AccessView（I0-5 T2，设计 §3：demo ui.ts 页面骨架的 Vue 化组装）——
 * 挂载拉取 /api/state、5s 审批轮询（demo L31）、2s 服务健康徽章（Home 退役承接）、
...
describe('AccessView 挂载与渲染', () => {
  ...
  it('onMounted 拉取 /api/state 渲染状态卡 + 安全边界静态卡 + 服务健康徽章（Home 退役承接）', async () => {
    ...
    expect(text).toContain('请先登录')
    // 安全边界四行（demo L17 原样）
    expect(text).toContain('Keycloak OIDC + PKCE')
    expect(text).toContain('按组织路径授权管理员')
    expect(text).toContain('ES256 · AES-GCM 加密落盘')
    expect(text).toContain('private_key_jwt + Bearer Token')
    // 健康徽章（fetchHealthz → interpretHealth/versionLineGated）
  ...
  it('fetch 失败（/api/state 不可达）→ 显示不可达文案而非白屏，页面骨架仍在', async () => {
    stubFetch({
