...
describe('PlatformAccessService——生产语义', () => {
  ...
  it('/auth/callback 全链：建会话 Set-Cookie + 302 / + 自动提交申请', async () => {
    ...
    const callbackRes = await service.callback({ ...HOST_CTX, path: '/auth/callback', query: new URLSearchParams({ code: 'auth-code', state: stateParam }) })
  ...
  it('callback 流程无效（state 不存在）→ 401', async () => {
    const service = serviceFor(tempProfile('http://p:18000'))
    const res = await service.callback({ ...HOST_CTX, path: '/auth/callback', query: new URLSearchParams({ code: 'x', state: 'nope' }) })
