 ...
 * - 024：间隔可配置（heartbeat.intervalSeconds，默认 60s），排程加 ±10% 随机抖动避免齐发
...
export interface HeartbeatDeps {
  ...
  /** 024：心跳间隔（毫秒），配置文件 heartbeat.intervalSeconds 注入；缺省 60s */
  intervalMs?: () => number
}
...
export class HeartbeatScheduler {
  ...
  /** 常态间隔：配置值（±10% 抖动）；未注入/非法回退 60s */
  private nextIntervalDelay(): number {
    let base = DEFAULT_INTERVAL_MS
    try {
      const configured = this.deps.intervalMs?.()
      if (typeof configured === 'number' && Number.isFinite(configured) && configured >= 30_000) {
        base = configured
      }
    } catch {
      // 配置读取失败回退默认
