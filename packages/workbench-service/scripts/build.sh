...
#      - workbench.exe        console 子系统（CLI 面：status/stop 需要控制台输出）
#      - workbench-daemon.exe GUI 子系统（--windows-hide-console，计划任务守护专用——
...
bun build --compile \
  ...
  --windows-publisher "Placeholder" \
  --windows-version "${WB_VERSION}" \
  src/main.ts --outfile dist/workbench-daemon.exe
# Bun 1.3.9 的 --windows-hide-console 实测不翻 PE Subsystem（产物仍 CUI）——后处理翻字节：
# CLI 子系统编译（保住 VersionInfo 旗标效果）+ 翻 GUI 子系统 = 零闪现守护变体
bun run scripts/flip-subsystem.ts dist/workbench-daemon.exe dist/workbench-daemon.exe

echo "built: cli=$(stat -c%s dist/workbench.exe) daemon=$(stat -c%s dist/workbench-daemon.exe) bytes"
