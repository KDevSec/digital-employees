...
**底座在场（V0.1，D-bb01）**：本机是否安装某底座 = 该底座 CLI `--version` 能跑通，不探测配置目录。未装时工作台只对**已登记**底座执行 `npm install -g`。自定义/未登记底座仍走 `/api/bases/register`（V0.2）。页面默认只展示 CodeBuddy 与 Qoder（D-029）。
...
## §4 产品命名规范（2026-08-26 用户裁决）

- **对外正式名称统一为「研发零处数字员工终端」**——Web 页面一切正式名称位置（浏览器标题/登录页主标/侧栏 logo 悬停提示/页面品牌位）一律使用该名称，不得以 DevZero 作为正式名称出现。
- DevZero 仅为内部代号（service brand.ts 契约字段/托盘 VersionInfo/安装包制品名等跨制品资产沿用），不在 Web UI 文案出现。
- Web 页面其余文案用词：「终端」替代「工作台」（如「终端 ID」「发送终端心跳」）；按钮文案精简（登录按钮即「登录」）。
- 命名变更走本节修订，不散落各任务自行其是。
